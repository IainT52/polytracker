...
const signalEngine_1 = require("./engine/signalEngine");
const db_1 = require("./db");
const schema_1 = require("./db/schema");
async function verifyEngine() {
    console.log('--- Phase 3 Verification: Scoring & Alert Engine ---');
    // 1. Setup Mock DB State
    const [mockMarket] = await db_1.db.insert(schema_1.markets).values({
        conditionId: '0xMockMarketEngine123',
        question: 'Will AI take over grading?',
        outcomes: '["Yes", "No"]'
    }).returning();
    const [wallet1] = await db_1.db.insert(schema_1.wallets).values({ address: '0xMockSmartMoneyA123', grade: 'D' }).returning();
    const [wallet2] = await db_1.db.insert(schema_1.wallets).values({ address: '0xMockSmartMoneyB456', grade: 'C' }).returning();
    ...
    const [trade1] = await db_1.db.insert(schema_1.trades).values({
        walletId: wallet1.id, marketId: mockMarket.id, outcomeIndex: 0, action: 'BUY', price: 0.40, shares: 100
    }).returning();
    await (0, signalEngine_1.processTradeForAlphaSignal)(trade1.id, wallet1.id, mockMarket.id, 0, 0.40, new Date());
    ...
    const [trade2] = await db_1.db.insert(schema_1.trades).values({
        walletId: wallet2.id, marketId: mockMarket.id, outcomeIndex: 0, action: 'BUY', price: 0.60, shares: 100
    }).returning();
    await (0, signalEngine_1.processTradeForAlphaSignal)(trade2.id, wallet2.id, mockMarket.id, 0, 0.60, new Date());
    ...
    const [trade3] = await db_1.db.insert(schema_1.trades).values({
        walletId: wallet2.id, marketId: mockMarket.id, outcomeIndex: 0, action: 'BUY', price: 0.45, shares: 100
    }).returning();
    await (0, signalEngine_1.processTradeForAlphaSignal)(trade3.id, wallet2.id, mockMarket.id, 0, 0.45, new Date());
