...
exports.mapSyndicates = mapSyndicates;
const db_1 = require("../db");
const drizzle_orm_1 = require("drizzle-orm");
const schema_1 = require("../db/schema");
async function mapSyndicates() {
    console.log('[SyndicateMapper] Starting N-Size Syndicate Graph Clustering...');
    const startTime = Date.now();
    try {
        // Step A: Fetch "Smart" trades
        const rawTrades = await db_1.db.all((0, drizzle_orm_1.sql) `
      SELECT 
        t.wallet_id,
        t.market_id,
        t.outcome_index,
    ...
    `);
        ...
        const addEdge = (w1, w2) => {
            if (!graph.has(w1))
                graph.set(w1, new Map());
            if (!graph.has(w2))
                graph.set(w2, new Map());
        ...
        await db_1.db.delete(schema_1.syndicateMembers);
        await db_1.db.delete(schema_1.syndicates);
        let clusterId = 1;
        for (const cluster of clusters) {
            ...
            const result = await db_1.db.insert(schema_1.syndicates).values({
                name,
                size: cluster.length,
            ...
            }).returning({ id: schema_1.syndicates.id });
            const insertedId = result[0].id;
            const memberRows = cluster.map(wallet => ({
                syndicateId: insertedId,
                walletAddress: wallet
            }));
            await db_1.db.insert(schema_1.syndicateMembers).values(memberRows);
        }
