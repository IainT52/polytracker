...
exports.runWalletGrader = runWalletGrader;
 ...
 */
async function runWalletGrader() {
    console.log('[Grader] Running SQL-Native Wallet Grading...');
    const thirtyDaysAgoMs = Date.now() - (30 * 24 * 60 * 60 * 1000);
    const rs = await db_1.client.execute({
        sql: `
    ...
    for (const stats of aggregatedStats) {
        ...
        if (totalTrades >= 30 && totalVolume >= 10000) {
            if (roi > 25 && winRate > 60)
                grade = 'A';
            else if (roi > 10 && winRate > 50)
                grade = 'B';
            else if (roi > 0)
                grade = 'C';
        }
    ...
    for (let i = 0; i < updates.length; i += CHUNK_SIZE) {
        ...
        const batchQueries = chunk.map(update => db_1.db.update(schema_1.wallets)
            .set({
            grade: update.grade,
            roi: update.roi,
